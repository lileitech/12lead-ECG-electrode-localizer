...
def testRootTimeEffect(threadsNum_val, meshName_val, conduction_speeds, rootNodeResolution, healthy_val, has_endocardial_layer_val):
    ...
    # EikonalDjikstraTet configuration
    global nlhsParam
...
#     nb_roots_range = [2, 5] # range of number of root nodes per endocardial chamber
    ...
    if 'atm' in target_type:
        experiment_output = 'atm'
    elif 'simulated_ecg_normalised' in target_type:
        experiment_output = 'simulated_ecg_normalised'
    ...
    if experiment_output == 'simulated_ecg_normalised' or experiment_output == 'bsp':
        global tetrahedrons
        ...
        elif experiment_output == 'simulated_ecg_normalised':
...
#     nb_roots_range = [2, 5] # range of number of root nodes per endocardial chamber
    ...
    if 'atm' in targetType:
        experiment_output = 'atm'
    elif 'simulated_ecg_normalised' in targetType:
        experiment_output = 'simulated_ecg_normalised'
    ...
    #     f.write('#\n# EikonalDjikstraTet generated post-process files\n# Ensight Gold Format\n#\n# Problem name:\theart\n#\n')
    ...
    #     f.write('EikonalDjikstraTet Ensight Gold --- Scalar per-node variables file\npart\n\t1\ncoordinates\n')
    ...
    #                 f.write('EikonalDjikstraTet Ensight Gold --- Scalar per-element variables file\npart\n\t1\ntetra4\n')
...
def test_dtw(population, population_2, meshName_val, meshVolume_val, threadsNum_val, experiment_output_val):
    ...
    # global root_node_activation_indexes
    rootNodesIndexes_true = (np.loadtxt(dataPath + meshName + '_coarse_rootNodes.csv') - 1).astype(int)
    rootNodesIndexes_true = np.unique(rootNodesIndexes_true)
#     root_node_activation_indexes = np.concatenate((lvActivationIndexes, rvActivationIndexes), axis=0)
#     nRootLocations=root_node_activation_indexes.shape[0]
#     nparam = nlhsParam + nRootLocations
...
#                                         atm2 = np.loadtxt(aux_fileName, delimiter=',')
                                    ...
                                    target_data.append({'simulated_ecg_normalised': pseudoBSP(np.loadtxt(aux_fileName, delimiter=',')),
                                    ...
                                    for endo_mult in [0.5, 1., 2]:
                                        for trans_mult in [0.5, 1., 2]:
                                            target_data.append({'simulated_ecg_normalised': eikonal_ecg(
                                                np.array([np.array([myoSpeed_true[0], myoSpeed_true[1]*trans_mult, myoSpeed_true[2],
                                                                ...
                                                                'model': 'eikonal', 'origin': 'true', 'source': 'simulated_ecg_normalised',
                                                                'endoTag': aux_endoSpeedTag, 'epiTag': aux_epiSpeedTag,
                                    ...
                                    for i in range(5):
                                        ...
                                        if outputType == 'atm' or outputType == 'all':
                                            ...
                                            if os.path.isfile(previousResultsPath + aux_fileName):
                                                ...
                                                target_data.append({'simulated_ecg_normalised': eikonal_ecg(np.array([params]), rootNodeActivationIndexes, rootNodesTimes)[0],
                                            ...
                                            if os.path.isfile(previousResultsPath + aux_fileName):
                                                ...
                                                target_data.append({'simulated_ecg_normalised': eikonal_ecg(np.array([pred_speeds]), rootNodes, rootNodesTimes)[0],
                                        ...
                                        # Store predictions with lowest discrepancies for simulated_ecg_normalised as target
                                        if outputType == 'simulated_ecg_normalised' or outputType == 'all':
                                            aux_fileName = (meshName + '_coarse_' + aux_endoSpeedTag + '_' + aux_epiSpeedTag
                                            ...
                                            if os.path.isfile(previousResultsPath + aux_fileName):
                                                ...
                                                params = df_roots[df_roots['fileName']==aux_fileName]['params'].to_numpy()[0]
                                                target_data.append({'simulated_ecg_normalised': eikonal_ecg(np.array([params]), rootNodeActivationIndexes, rootNodesTimes)[0],
                                                                    'model': 'eikonal', 'origin': 'pred', 'source': 'simulated_ecg_normalised',
                                                                    'endoTag': aux_endoSpeedTag, 'epiTag': aux_epiSpeedTag,
                                                                    'endo': params[3], 'trans': params[1], 'run': i})
                                            elif i==0:
                                                print(previousResultsPath + aux_fileName)
                                                print('simulated_ecg_normalised')
                                                raise

                                        

                                            # Store predictions using the ensamble infered parameters from the simulated_ecg_normalised results
                                            aux_fileName = (meshName + '_coarse_' + aux_endoSpeedTag + '_' + aux_epiSpeedTag + '_'
                                            ...
                                            if os.path.isfile(previousResultsPath + aux_fileName):
                                                ...
                                                pred_speeds = np.median(population[:, :4], axis=0)
                                                target_data.append({'simulated_ecg_normalised': eikonal_ecg(np.array([pred_speeds]), rootNodes, rootNodesTimes)[0],
                                                                    'model': 'eikonal', 'origin': 'median-pred', 'source': 'simulated_ecg_normalised',
                                                                    'endoTag': aux_endoSpeedTag, 'epiTag': aux_epiSpeedTag,
                                                                    'endo': pred_speeds[3], 'trans': pred_speeds[1], 'run': i})
    ...
    with pymp.Parallel(min(threadsNum, numConf)) as p3:
        for conf_i in p3.range(numConf):
            ...
            if os.path.isfile(figName) and not redoAll:
                ...
                max_count = 0
                if 'EikonalDjikstraTet-True' in line_names:
                    plot_width = 6
                else:
                    plot_width = 4
                ...
                for meshName_i in range(len(meshName_list)):
                    ...
                    for i in range(meshName_i*len(configurations), (meshName_i+1)*len(configurations)):
                        ...
                        x2 = auxdf['source']=='atm'
                        x1 = np.logical_and(x1, x2)
                        bidoEcg = auxdf[x1]['simulated_ecg_normalised'].to_numpy()[0]

                        # EikonalDjikstraTet True
                        if do_eikonal_line:
                            eikoEcg = []
                            for endo_mult in [1., 0.5, 2]:
                                for trans_mult in [1., 0.5, 2]:
                                    ...
                                    x2 = auxdf['origin']=='true'
                                    x1 = np.logical_and(x1, x2)
                                    x2 = auxdf['source']=='simulated_ecg_normalised'
                                    x1 = np.logical_and(x1, x2)
                                    x2 = auxdf['endo']==endo_mult
                                    x1 = np.logical_and(x1, x2)
                                    x2 = auxdf['trans']==trans_mult
                                    x1 = np.logical_and(x1, x2)
                                    eikoEcg.append(auxdf[x1]['simulated_ecg_normalised'].to_numpy()[0])
                        ...
                        if do_atm_pred_line:
                            ...
                            x2 = auxdf['source']=='atm'
                            x1 = np.logical_and(x1, x2)
                            eikoEcg_atm_pred = auxdf[x1]['simulated_ecg_normalised'].to_numpy()
                #             print(len(eikoEcg_atm_pred))
                        ...
                        if do_ecg_pred_line:
                            ...
                            x2 = auxdf['source']=='simulated_ecg_normalised'
                            x1 = np.logical_and(x1, x2)
                            eikoEcg_ecg_pred = auxdf[x1]['simulated_ecg_normalised'].to_numpy()
                #             print(len(eikoEcg_ecg_pred))
                        ...
                        if do_atm_inf_line:
                            ...
                            x2 = auxdf['source']=='atm'
                            x1 = np.logical_and(x1, x2)
                            eikoEcg_atm_inf = auxdf[x1]['simulated_ecg_normalised'].to_numpy()
                #             print(len(eikoEcg_atm_inf))
                        ...
                        if do_ecg_inf_line:
                            ...
                            x2 = auxdf['source']=='simulated_ecg_normalised'
                            x1 = np.logical_and(x1, x2)
                            eikoEcg_ecg_inf = auxdf[x1]['simulated_ecg_normalised'].to_numpy()
    ...
    for discretisation_resolution in discretisation_list:
        for target_type in target_type_list:
            if 'atm' in target_type:
                ...
                data_type = 'simulated_ecg_normalised'
            elif 'bsp' in target_type:
                data_type = 'bsp'
...
def clearPreviousOutputs(figPath, meshName):
    remove_file(figPath + meshName + '.ensi.PKtimes')
...
def mapAndGeneratePKTree(meshName, figPath):
    clearPreviousOutputs(figPath, meshName)
...
#add a little of buffer time
    ...
    if os.path.isfile(figPath+meshName+'.ensi.case'):
        with open(figPath+meshName+'.ensi.case', 'a') as f:
            ...
            f.write('scalar per node: 1	PK_times	' + meshName + '.ensi.PKtimes\n')
    else:
        with open(figPath+meshName+'.ensi.case', 'w') as f:
            f.write('#\n# EikonalDjikstraTet generated post-process files\n# Ensight Gold Format\n#\n# Problem name:\theart\n#\n')
            f.write('FORMAT\ntype:\tensight gold\nGEOMETRY\nmodel:\t1\t'+meshName+'.ensi.geo\nVARIABLE\n')
    ...
    #
    # # Target result
    # global reference_lead_is_positive # 22/05/03 - Normalise also the limb leads to give them relative importance
    ...
    #     raise()
    #
    # reference_lead_is_positive = reference_lead_is_max_aux # 22/05/03 - Have some R progression by normalising by the largest absolute amplitude lead
    #
    #
...
def writeOutEnsigh(meshName, figPath):
    # Paths and tags
    dataPath = 'metaData/' + meshName + '/'
    # Load mesh
    nodesXYZ = np.loadtxt(dataPath + meshName + '_xyz.csv', delimiter=',')
...
def evaluateEikonal_volume_no_fields(threadsNum_val, meshName, conduction_speeds, resolution, figPath, healthy_val,
...
#add a little of buffer time
    ...
    print('tVolumes')
    print(tVolumes.shape)
    # print('r')
    # print(r.shape)
    # print(np.amin(r))
    # print(np.amax(r))
    # print(tetrahedronCenters.shape)
    # print(nodesXYZ.shape)
    distances_to_electrodes = np.linalg.norm(r, axis=2)
    per_electrode_min_distance = np.amin(distances_to_electrodes, axis=0)
    per_electrode_max_distance = np.amax(distances_to_electrodes, axis=0)
    distances_to_electrodes = (distances_to_electrodes - per_electrode_min_distance)/(per_electrode_max_distance - per_electrode_min_distance)
    # print(per_electrode_min_distance.shape)
    # print(per_electrode_min_distance)
    # for i in range(distances_to_electrodes.shape[1]):
    
    # print(distances_to_electrodes.shape)
    # print(np.amin(distances_to_electrodes, axis=0))
    # print(np.amax(distances_to_electrodes, axis=0))
    ...
    distances_to_precordial_electrodes = distances_to_electrodes[:, 4:]
    # print(distances_to_precordial_electrodes.shape)
    # raise
    ...
    fileName = figPath + meshName + '_clinical_inferred_particles.csv'
    distance_kept_ratio = np.linspace(0.1, 1., 5)
    print('distance_kept_ratio')
    print(distance_kept_ratio)
    if os.path.isfile(fileName):
        print(fileName)
        population = np.loadtxt(fileName, delimiter=',')
        # fig, axs = plt.subplots(nrows=population.shape[0], ncols=nb_leads-2, constrained_layout=True, figsize=(40, 5*population.shape[0]), sharey='row')
        inference_ecgs = eikonal_ecg(population, rootNodeActivationIndexes, rootNodeActivationTimes)
        # cmap = plt.get_cmap('viridis')
        # colours = cmap(np.linspace(0, 1, len(distance_kept_ratio)))
        
        
        for consistency_i in range(population.shape[0]):
            max_count = 0
            fig, axs = plt.subplots(nrows=distance_kept_ratio.shape[0], ncols=nb_leads-2, constrained_layout=True, figsize=(40, 5*distance_kept_ratio.shape[0]), sharey='row')
            inference_atmap = eikonal_atm(population[consistency_i, :], rootNodeActivationIndexes, rootNodeActivationTimes)
            ATMap = inference_atmap.astype(int)
            for distance_ratio_i in range(len(distance_kept_ratio)):
            # for i in range(distance_kept_ratio.shape[0]):
            #     for j in range(nb_leads-2):
                
            
                for precordial_i in range(nb_leads-2):
                    axs_res = axs[distance_ratio_i, precordial_i]
                    # axs_res.plot(inference_ecgs[consistency_i, precordial_i, :2], 'k-', label='Reference', linewidth=2)
                    # axs_res.plot(inference_ecgs[consistency_i, precordial_i, :2], 'b-', label=str(distance_kept_ratio[distance_ratio_i]), linewidth=2)
                
                    # axs_res = axs[distance_ratio_i, precordial_i]
                    reference_lead = inference_ecgs[consistency_i, precordial_i+2, :]
                    reference_lead = reference_lead[np.logical_not(np.isnan(reference_lead))]
                    max_count = max(max_count, np.sum(np.logical_not(np.isnan(reference_lead))))
                    axs_res.plot(reference_lead, 'k-', label='Reference', linewidth=2)
                    # title_text = None
                
                    elem_mask = np.zeros((tetrahedrons.shape[0]), dtype=bool)
                    # print(np.sum(elem_mask))
                    elem_mask[distances_to_precordial_electrodes[:, precordial_i]<=distance_kept_ratio[distance_ratio_i]] = 1
                    
                    new_volume = np.sum(tVolumes[elem_mask])
                    ...
                    for timestep_i in range(1, nb_timesteps, 1):
                        ...
                        if not len(activeNodes) == 0:
                            ...
                            masked_activeEle = elem_mask[activeEle]
                            # print('activeEle')
                            # print(activeEle)
                            # print(activeEle.shape)
                            # print('masked_activeEle')
                            # print(masked_activeEle.shape)
                            # print(masked_activeEle)
                            activeEle = activeEle[masked_activeEle.astype(bool)]
                            # print(activeEle.shape)
                            # print(activeEle)
                            if not len(activeEle) == 0:
                    ...
                    ECG = ECG - (ECG[:, 0:1]+ECG[:, -2:-1])/2 # align at zero # 2022/05/03
                   
                    reference_amplitudes = np.empty(shape=(nb_leads), dtype=np.float64) # 2022/05/04
                    reference_amplitudes[reference_lead_is_max] = np.amax(ECG, axis=1)[reference_lead_is_max]
                    reference_amplitudes[np.logical_not(reference_lead_is_max)] = np.absolute(np.amin(ECG, axis=1))[np.logical_not(reference_lead_is_max)]
                    ECG[:2, :] = ECG[:2, :] / np.mean(reference_amplitudes[:2]) # 22/05/03
                    ECG[2:nb_leads, :] = ECG[2:nb_leads, :] / np.mean(reference_amplitudes[2:nb_leads]) # 22/05/03 TODO: Uncomment
                    
                    precordial_lead = ECG[precordial_i+2, :]
                    precordial_lead = precordial_lead[np.logical_not(np.isnan(precordial_lead))]
                    max_count = max(max_count, np.sum(np.logical_not(np.isnan(precordial_lead))))
                    
                    if precordial_lead.shape[0] >= reference_lead.shape[0]:
                        a = precordial_lead
                        b = reference_lead
                    else:
                        a = reference_lead
                    ...
                    #     title_text = str(round(new_corr_value, 2))
                    # else:
                    title_text = 'v:'+ str(round(new_volume, 2)) + '\n' + 'cc:' + str(round(new_corr_value, 2))
                    
                    # axs_res = axs[consistency_i, precordial_i]
                    axs_res.plot(precordial_lead, 'b-', label=str(distance_kept_ratio[distance_ratio_i]), linewidth=2)
                    # axs_res.plot(reference_lead, 'k-', linewidth=2.)
                    ...
                    axs_res.set_title(title_text, fontsize=16)
            ...
            figName = (figPath + meshName + '_volume_evaluation_'+str(consistency_i)+'.png')
    ...
    if True:
        if True:
            ...
            distance_kept = np.linspace(0.1, 1., threadsNum)
            rng = np.random.default_rng(seed=123) # random number generator
            bdate = pandas.bdate_range(start='2019-01-01', end='2021-07-24')
            daily_value_change = rng.normal(loc=0, scale=0.005, size=bdate.size)
            value = 1.1 + np.cumsum(daily_value_change)
            euro_to_dollar = pandas.DataFrame(dict(Time=bdate, US_dollar=value))
            ...
            for parameter_i in range(4):
                ...
                # inference_atmap = eikonal_atm(population[best_id, :], root_node_activation_indexes, rootNodeActivationTimes) # 2022/01/18 - Test the plot of the the best CC instead of the kmeans particle.
                ...
                for i in range(nb_leads):
                    ...
                    for j in range(1, prediction_list.shape[0]):
                        prediction_lead = prediction_list[j, i, :]
                        lead_size = np.sum(np.logical_not(np.isnan(prediction_lead)))
                    ...
                    prediction_lead = prediction_list[1, i, :]
                    lead_size = np.sum(np.logical_not(np.isnan(prediction_lead)))
                    prediction_lead = prediction_lead[:lead_size]
...
# EVALUATE EIKONAL ECGS USING INFERENCE RESULTS AS BASELINE 2022/05/19 - IDEAL FOR SENSITIVITY TESTING TO PARAMETER CHANGES
def evaluateEikonal_volume_backup(threadsNum_val, meshName, resolution, figPath, healthy_val, load_target, target_data_path, endocardial_layer):
    purkinje_speed = 0.2 # cm/ms # the final purkinje network will use 0.19 but we don't need to use a slightly slower speed to
#allow for curved branches in the networks, because the times are here computed on a jiggsaw-like coarse mesh which will already
#add a little of buffer time
    ...
    fileName = figPath + meshName + '_clinical_inferred_particles.csv'
    # distance_kept_ratio = np.linspace(0.1, 1., 10)
    distance_kept_ratio = np.linspace(1., 0.2, 100)
    print('distance_kept_ratio')
    print(distance_kept_ratio)
    if os.path.isfile(fileName):
        print(fileName)
        ...
        for consistency_i in range(population.shape[0]):
            ...
            inference_atmap = eikonal_atm(population[consistency_i, :], rootNodeActivationIndexes, rootNodeActivationTimes)
            ATMap = inference_atmap.astype(int)
            # elem_visited_ids = np.zeros((nb_leads-2, tetrahedrons.shape[0]), dtype=bool)
            elem_visited_values = np.zeros((nb_leads-2, tetrahedrons.shape[0]), dtype=float)
            prev_pcc = np.full((nb_leads-2), np.nan, np.float64) # np.nan needs to be float64 or it won't be recognised by the function np.isnan()
            for distance_ratio_i in range(len(distance_kept_ratio)):
                for precordial_i in range(nb_leads-2):
                    axs_res = axs[distance_ratio_i, precordial_i]
                    reference_lead = inference_ecgs[consistency_i, precordial_i+2, :]
                    ...
                    # elem_mask[distances_to_precordial_electrodes[:, precordial_i]<=distance_kept_ratio[distance_ratio_i]] = 1
                    # elem_mask[distances_to_precordial_electrodes[:, precordial_i]>=distance_kept_ratio[distance_ratio_i]] = 1
                    # elem_visited_new_ids = np.logical_and(distances_to_precordial_electrodes[:, precordial_i]<=distance_kept_ratio[distance_ratio_i], np.logical_not(elem_visited_ids[precordial_i, :]))
                    # elem_visited_new_ids = distances_to_precordial_electrodes[:, precordial_i]<=distance_kept_ratio[distance_ratio_i]
                    # print('np.sum(np.abs(elem_mask-elem_visited_new_ids))')
                    # print(np.sum(np.abs(np.logical_xor(elem_mask,elem_visited_new_ids))))
                    elem_mask = distances_to_precordial_electrodes[:, precordial_i]<=distance_kept_ratio[distance_ratio_i]
                    # print('np.sum(np.abs(elem_mask-elem_visited_new_ids))')
                    # print(np.sum(np.abs(elem_mask-elem_visited_new_ids)))
                    # elem_visited_new_ids = np.logical_and(distances_to_precordial_electrodes[:, precordial_i]>=distance_kept_ratio[distance_ratio_i], np.logical_not(elem_visited_ids[precordial_i, :]))
                    # print('np.sum(elem_visited_mask_new_ids)')
                    # print(np.sum(elem_visited_mask_new_ids))
                    ...
                    if not np.isnan(prev_pcc[precordial_i]):
                        # print()
                        # print(prev_pcc[precordial_i])
                        # print(np.isnan(prev_pcc[precordial_i]))
                        # print(np.isnan(prev_pcc))
                        new_corr_value = min(new_corr_value, prev_pcc[precordial_i])
                    prev_pcc[precordial_i] = new_corr_value
                    
                    # print('Lead: '+ str(precordial_i)+' Volume: '+ str(new_volume) + ' PCC: ' + str(round(new_corr_value, 2)))
                    
                    # elem_visited_ids[precordial_i, elem_visited_new_ids] = 1
                    elem_visited_values[precordial_i, elem_mask] = new_corr_value
            ...
            # Save PCC field for each lead
            for precodrial_i in range(nb_leads-2):
                with open(figPath + meshName + '_precordial_' + str(precodrial_i+1) + '_' +  str(consistency_i) + '.ensi.PCC_bands', 'w') as f:
                    f.write('EikonalDjikstraTet Ensight Gold --- Scalar per-element variables file\npart\n\t1\ntetra4\n')
                    for i in range(0, elem_visited_values.shape[1]):
                        f.write(str(elem_visited_values[precodrial_i, i])+'\n')
                        
            # create heart.ensi.case file
            with open(figPath+meshName+'_volume.ensi.case', 'w') as f:
                f.write('#\n# EikonalDjikstraTet generated post-process files\n# Ensight Gold Format\n#\n# Problem name:\theart\n#\n')
                f.write('FORMAT\ntype:\tensight gold\nGEOMETRY\nmodel:\t1\t'+meshName+'.ensi.geo\nVARIABLE\n')
                for precodrial_i in range(nb_leads-2):
                    f.write('scalar per element: 1	PCC_bands_precordial_V' + str(precodrial_i+1) + '	' + meshName + '_precordial_' + str(precodrial_i+1) + '_' +  str(consistency_i) +
                    '.ensi.PCC_bands\n')
    else:
        print('File :'+ fileName + ', doesnt exist.')
        raise
...
# EVALUATE EIKONAL ECGS USING INFERENCE RESULTS AS BASELINE 2022/05/19 - IDEAL FOR SENSITIVITY TESTING TO PARAMETER CHANGES
def evaluateEikonal_volume(threadsNum_val, meshName, resolution, figPath, healthy_val, load_target, target_data_path, endocardial_layer):
    purkinje_speed = 0.2 # cm/ms # the final purkinje network will use 0.19 but we don't need to use a slightly slower speed to
#allow for curved branches in the networks, because the times are here computed on a jiggsaw-like coarse mesh which will already
#add a little of buffer time
    ...
    fileName = figPath + meshName + '_clinical_inferred_particles.csv'
    # distance_kept_ratio = np.linspace(0.1, 1., 10)
    distance_kept_ratio = np.linspace(1., 0.01, 100)
    print('distance_kept_ratio')
    print(distance_kept_ratio)
    if os.path.isfile(fileName):
        print(fileName)
        ...
        for consistency_i in range(population.shape[0]):
            ...
            inference_atmap = eikonal_atm(population[consistency_i, :], rootNodeActivationIndexes, rootNodeActivationTimes)
            ATMap = inference_atmap.astype(int)
            elem_corr_values = np.zeros((nb_leads-2, tetrahedrons.shape[0]), dtype=float)
            elem_diff_values = np.zeros((nb_leads-2, tetrahedrons.shape[0]), dtype=float)
            prev_pcc = np.full((nb_leads-2), np.nan, np.float64) # np.nan needs to be float64 or it won't be recognised by the function np.isnan()
            prev_diff = np.full((nb_leads-2), np.nan, np.float64) # np.nan needs to be float64 or it won't be recognised by the function np.isnan()
            for distance_ratio_i in range(len(distance_kept_ratio)):
                for precordial_i in range(nb_leads-2):
                    # axs_res = axs[distance_ratio_i, precordial_i]
                    reference_lead = inference_ecgs[consistency_i, precordial_i+2, :]
                    ...
                    b = b_aux
                    new_corr_value = np.corrcoef(a, b)[0,1]
                    diff_values = np.clip(np.abs((a-b)/np.clip(reference_lead, 0.1, 1.)), 0., 1.)
                    # print('diff_values.shape')
                    # print(diff_values.shape)
                    # print(np.amin(diff_values))
                    # print(np.amax(diff_values))
                    new_diff_value = 1. - np.mean(diff_values)
                    
                    if not np.isnan(prev_pcc[precordial_i]):
                        new_corr_value = min(new_corr_value, prev_pcc[precordial_i])
                    prev_pcc[precordial_i] = new_corr_value
                    elem_corr_values[precordial_i, elem_mask] = new_corr_value
                    
                    if not np.isnan(prev_diff[precordial_i]):
                        new_diff_value = max(new_diff_value, prev_diff[precordial_i])
                    prev_diff[precordial_i] = new_diff_value
                    elem_diff_values[precordial_i, elem_mask] = new_diff_value
            ...
            for precodrial_i in range(nb_leads-2):
                with open(figPath + meshName + '_precordial_' + str(precodrial_i+1) + '_' +  str(consistency_i) + '.ensi.PCC_bands', 'w') as f:
                    f.write('EikonalDjikstraTet Ensight Gold --- Scalar per-element variables file\npart\n\t1\ntetra4\n')
                    for i in range(0, elem_corr_values.shape[1]):
                        f.write(str(elem_corr_values[precodrial_i, i])+'\n')
            # Save DIFF field for each lead
            for precodrial_i in range(nb_leads-2):
                with open(figPath + meshName + '_precordial_' + str(precodrial_i+1) + '_' +  str(consistency_i) + '.ensi.DIFF_bands', 'w') as f:
                    f.write('EikonalDjikstraTet Ensight Gold --- Scalar per-element variables file\npart\n\t1\ntetra4\n')
                    for i in range(0, elem_diff_values.shape[1]):
                        f.write(str(elem_diff_values[precodrial_i, i])+'\n')
            ...
            with open(figPath+meshName+'_volume.ensi.case', 'w') as f:
                ...
                for precodrial_i in range(nb_leads-2):
                    f.write('scalar per element: 1	PCC_bands_precordial_V' + str(precodrial_i+1) + '	' + meshName + '_precordial_' + str(precodrial_i+1) + '_' +  str(consistency_i) +
                    '.ensi.PCC_bands\n')
                    f.write('scalar per element: 1	DIFF_bands_precordial_V' + str(precodrial_i+1) + '	' + meshName + '_precordial_' + str(precodrial_i+1) + '_' +  str(consistency_i) +
                    '.ensi.DIFF_bands\n')
    else:
        print('File :'+ fileName + ', doesnt exist.')
        raise
...
# PREPARE RESULTS FOR LUCAS ALGORITHM 2022/07/12
# -  Location and LAT of the root nodes in the LV and RV.
# -  Location of the "dense" and "sparse" regions.
def generate_results_for_lucas(meshName, figPath, resultPath, healthy_val, data_type, metric, endocardial_layer):
    target_type = data_type+'_'+metric
    global has_endocardial_layer
    has_endocardial_layer = endocardial_layer
    # 02/12/2021 remove the healthy case because the anisotropy of the wavefront will strongly depend on the fibre orientation planes with respect to the endocardial wall
    ...
    # Load results from the inference and save them for Lucas to use later
    fileName = resultPath + meshName + '_clinical_inferred_particles.csv'
    if os.path.isfile(fileName):
        print(fileName)
        population = np.loadtxt(fileName, delimiter=',')
        for consistency_i in range(population.shape[0]):
            # reference_parameter_set = population[consistency_i, :]
            lv_times = []
            rv_times = []
            lv_xyz = []
            rv_xyz = []
            fileName = resultPath + meshName + '_' + str(consistency_i) + '_inferred_root_nodes_times.csv'
            rootTimes = np.loadtxt(fileName, delimiter=',')
            fileName = resultPath + meshName + '_' + str(consistency_i) + '_inferred_root_nodes_xyz.csv'
            rootXYZ = np.loadtxt(fileName, delimiter=',')
            for root_i in range(rootXYZ.shape[0]):
                root_node_aux = rootXYZ[root_i, :]
                root_node_time_aux = rootTimes[root_i]
                if np.amin(np.linalg.norm(nodesXYZ[lvnodes, :]-root_node_aux, axis=1)) == 0:
                    lv_xyz.append(root_node_aux)
                    lv_times.append(root_node_time_aux)
                elif np.amin(np.linalg.norm(nodesXYZ[rvnodes, :]-root_node_aux, axis=1)) == 0:
                    rv_xyz.append(root_node_aux)
                    rv_times.append(root_node_time_aux)
                else:
                    print('bad')
                    print(np.amin(np.linalg.norm(nodesXYZ[lvnodes, :]-root_node_aux, axis=1)))
                    print(np.amin(np.linalg.norm(nodesXYZ[rvnodes, :]-root_node_aux, axis=1)))
                    raise
            fileName = figPath + meshName + '_' + str(consistency_i) + '_inferred_root_nodes_xyz_lv.csv'
            with open(fileName, 'w') as f:
                for i in range(0, len(lv_xyz)):
                    aux_val = lv_xyz[i]
                    text = str(aux_val[0])
                    for j in range(1, len(aux_val)):
                        text = text + ',' + str(aux_val[j])
                    f.write(text + '\n')
            fileName = figPath + meshName + '_' + str(consistency_i) + '_inferred_root_nodes_times_lv.csv'
            with open(fileName, 'w') as f:
                for i in range(0, len(lv_times)):
                    aux_val = lv_times[i]
                    text = str(aux_val)
                    f.write(text + '\n')
            fileName = figPath + meshName + '_' + str(consistency_i) + '_inferred_root_nodes_xyz_rv.csv'
            with open(fileName, 'w') as f:
                for i in range(0, len(rv_xyz)):
                    aux_val = rv_xyz[i]
                    text = str(aux_val[0])
                    for j in range(1, len(aux_val)):
                        text = text + ',' + str(aux_val[j])
                    f.write(text + '\n')
            fileName = figPath + meshName + '_' + str(consistency_i) + '_inferred_root_nodes_times_rv.csv'
            with open(fileName, 'w') as f:
                for i in range(0, len(rv_times)):
                    aux_val = rv_times[i]
                    text = str(aux_val)
                    f.write(text + '\n')
    ...
    isDense = np.zeros((nodesXYZ.shape[0]), dtype=int)
    isSparse = np.ones((nodesXYZ.shape[0]), dtype=int)
    isDense[lv_dense_indexes] = 1
    isDense[rv_dense_indexes] = 1
    isSparse[lv_dense_indexes] = 0
    isSparse[rv_dense_indexes] = 0
    
    with open(figPath + meshName + '_' + target_type + '.ensi.DenseNodes', 'w') as f:
        f.write('EikonalDjikstraTet Ensight Gold --- Scalar per-node variables file\npart\n\t1\ncoordinates\n')
        for i in range(0, len(isDense)):
            f.write(str(isDense[i]) + '\n')
    with open(figPath + meshName + '_' + target_type + '.ensi.SparseNodes', 'w') as f:
        f.write('EikonalDjikstraTet Ensight Gold --- Scalar per-node variables file\npart\n\t1\ncoordinates\n')
        for i in range(0, len(isSparse)):
            f.write(str(isSparse[i]) + '\n')
    ...
    with open(figPath+meshName+'.ensi.case', 'w') as f:
        f.write('#\n# EikonalDjikstraTet generated post-process files\n# Ensight Gold Format\n#\n# Problem name:\theart\n#\n')
        f.write('FORMAT\ntype:\tensight gold\nGEOMETRY\nmodel:\t1\t'+meshName+'.ensi.geo\nVARIABLE\n')
        f.write('scalar per node: 1	DenseNodes	' + meshName + '_' + target_type + '.ensi.DenseNodes\n') # 2022/01/11
        f.write('scalar per node: 1	SparseNodes	' + meshName + '_' + target_type + '.ensi.SparseNodes\n') # 2022/01/11
    ...
    # compilation_params = np.array([np.concatenate((np.full(nlhsParam+1, 0.09), np.ones(root_node_activation_indexes.shape).astype(int)))]) # Compile Numba
    # eikonal_ecg_2(compilation_params, root_node_activation_indexes, np.zeros(root_node_activation_indexes.shape)) # Compile Numba # 02/12/2021
    #
    # fileName = figPath + meshName + '_clinical_inferred_particles.csv'
    # if os.path.isfile(fileName):
    #     print(fileName)
    ...
    #         fig, axs = plt.subplots(nrows=4, ncols=nb_leads, constrained_layout=True, figsize=(40, 24), sharey='all')
    #         mult_factor = np.linspace(0.5, 2., threadsNum-1)
    #         speed_names = ['Purkinje', 'sheet', 'DenseEndo', 'SparseEndo']
    #         for parameter_i in range(4):
    #             parameter_sets_with_purkinje = np.zeros((threadsNum, reference_parameter_set.shape[0]+1))
    #             parameter_sets_with_purkinje[:, 0] = purkinje_speed
    #             parameter_sets_with_purkinje[:, 1:] = np.transpose(np.reshape(np.repeat(reference_parameter_set, threadsNum, axis=0), (reference_parameter_set.shape[0], threadsNum)))
    #             if parameter_i > 0:
    #                 new_speeds = mult_factor*reference_parameter_set[parameter_i-1]
    #             else:
    #                 new_speeds = np.linspace(0.1, 2., threadsNum-1)
    #             parameter_sets_with_purkinje[1:, parameter_i] = new_speeds
    #
    #             # axs_hist[0, parameter_i].set_title(speed_names[parameter_i])
    #             if parameter_i == 0:
    #                 pk_speed = np.amax(new_speeds) # time is in ms
    ...
    #             rootNodeDistances = np.around(np.concatenate((lv_PK_distance_mat[lvActnode_ids], rv_PK_distance_mat[rvActnode_ids]), axis=0), decimals=4)
    #             prediction_list = eikonal_ecg_2(parameter_sets_with_purkinje, root_node_activation_indexes, rootNodeDistances)
    #             # Compute correlation coefficients to all ECGs in the unique parameter_sets_with_purkinje
    #             corr_mat = np.zeros((prediction_list.shape[0]-1, nb_leads)) # TODO - Uncomment 2022/05/09
    #             reference_ecg = prediction_list[0, :, :]
    #             reference_ecg = reference_ecg[:, np.logical_not(np.isnan(reference_ecg[0, :]))]
    #             for j in range(1, prediction_list.shape[0]):
    ...
    #                     lead_size = np.sum(np.logical_not(np.isnan(prediction_lead)))
    #                     prediction_lead = prediction_lead[:lead_size]
    #                     if new_speeds[j-1] <= parameter_sets_with_purkinje[0, parameter_i]:
    #                         axs_res.plot(prediction_lead, 'b-', linewidth=.3) # September 2021
    #                     else:
    #                         axs_res.plot(prediction_lead, 'r-', linewidth=.3) # September 2021
    #                 axs_res.plot(prediction_lead, 'm-', linewidth=2.) # largest value
...
# EVALUATE EIKONAL ECGS USING INFERENCE RESULTS AS BASELINE 2022/05/19 - IDEAL FOR SENSITIVITY TESTING TO PARAMETER CHANGES
def evaluateEikonal_speed(threadsNum_val, meshName, conduction_speeds, resolution, figPath, healthy_val,
                load_target, data_type, metric, target_data_path, endocardial_layer, is_clinical, experiment_path_tag,
                min_possible_consistency_count=0, max_possible_consistency_count = 100):
    purkinje_speed = 0.2 # cm/ms # the final purkinje network will use 0.19 but we don't need to use a slightly slower speed to
#allow for curved branches in the networks, because the times are here computed on a jiggsaw-like coarse mesh which will already
    ...
    if os.path.isfile(fileName):
        ...
        #             print(fileName)
            ...
            for parameter_i in range(4):
                ...
                axs_hist[0, parameter_i].hist(not_used_pk_times_aux)
                rootNodeDistances = np.around(np.concatenate((lv_PK_distance_mat[lvActnode_ids], rv_PK_distance_mat[rvActnode_ids]), axis=0), decimals=4)
                prediction_list = eikonal_ecg_2(parameter_sets_with_purkinje, rootNodeActivationIndexes, rootNodeDistances)
                # Compute correlation coefficients to all ECGs in the unique parameter_sets_with_purkinje
                corr_mat = np.zeros((prediction_list.shape[0]-1, nb_leads)) # TODO - Uncomment 2022/05/09
                reference_ecg = prediction_list[0, :, :]
                reference_ecg = reference_ecg[:, np.logical_not(np.isnan(reference_ecg[0, :]))]
...
#add a little of buffer time
        ...
        if resolution_count > 0:
            ...
            for resolution_i in range(len(aux_resolution_list)):
                ...
                if os.path.isfile(fileName):
                    ...
                    # prediction_list = eikonal_ecg(population, root_node_activation_indexes, rootNodeActivationTimes)
                    # best_ecg = prediction_list[best_id, :, :] # 2022/01/18
                    # best_ecg = best_ecg[:, np.logical_not(np.isnan(best_ecg[0, :]))]
                    # inference_ecg = best_ecg # TODO fix this # 2022/01/18
                    best_ecg_CC = prediction_list[best_id, :, :]
                    ...
                    # # best_ecg = eikonal_ecg(np.array([best_params]), root_node_activation_indexes, rootNodeActivationTimes) # 06/12/2021 - Test the plot of the the best CC instead of the kmeans particle.
                    # # aux_best_ecg = eikonal_ecg(best_params[np.newaxis, :], root_node_activation_indexes, rootNodeActivationTimes) # 06/12/2021 - Test the plot of the the best CC instead of the
                    # # kmeans particle.
                    ...
                    with open(figPath + meshName + '_' + str(consistency_i) + '_cummrootNodes.csv', 'w') as f:
                        for i in range(0, len(atmap)):
                            f.write(str(atmap[i])+'\n')
                            
                    atmap = np.zeros((nodesXYZ.shape[0]))
                    atmap[rootNodeActivationIndexes[np.asarray(population[best_id, nlhsParam:], dtype=int)]] = 1
                    with open(figPath + meshName + '_' + str(consistency_i) + '_inferred_rootNodes_map.csv', 'w') as f:
                        for i in range(0, len(atmap)):
                            f.write(str(atmap[i])+'\n')
                    ...
                    for i in range(nb_leads):
                        ...
                        # TODO: Uncomment next line to compare the ECG with the best discrepancy with the one with the best CC
                        ...
                        # for j in range(prediction_list.shape[0]):
                        #     prediction_lead = prediction_list[np.argmax(corr_list), i, :]
    ...
    for i in range(nb_leads):
        ...
        for j in range(len(result_summary)):
            ...
            # axs_res.plot(result_ecg[i, :], colour_list[j]+'-', label='Best-CC-'+str(j), linewidth=1.2) # 2021/12/15 - TODO Comment
...
#add a little of buffer time
    ...
    # eikonal_ecg(compilation_params, root_node_activation_indexes, rootNodeActivationTimes) # Compile numba function
    ...
    # print('fix this funciton and also the stopping criteria in simulated_ecg_normalised and bsp')
...
def save_cobiveco(figPath, meshName, nodesXYZ, paraview_elems, cobiveco_mapped):
    ...
    # Save cobiveco to be read into Matlab
    if not os.path.isfile(figPath + meshName+'_xyz.csv'):
        with open(figPath + meshName+'_xyz.csv', 'w') as f:
            for i in range(0, len(nodesXYZ)):
                f.write(str(nodesXYZ[i,0])+','+str(nodesXYZ[i,1])+','+str(nodesXYZ[i,2])+'\n')
    if not os.path.isfile(figPath + meshName+'_tri.csv'):
        with open(figPath + meshName+'_tri.csv', 'w') as f:
            for i in range(0, len(paraview_elems)):
                f.write(str(paraview_elems[i,0])+','+str(paraview_elems[i,1])+','+str(paraview_elems[i,2])+','+str(paraview_elems[i,3])+'\n')
    with open(figPath + meshName +  '_ab.csv', 'w') as f:
        for i in range(cobiveco_mapped.shape[0]):
            f.write(str(cobiveco_mapped[i, 0]) + '\n')
    with open(figPath + meshName +  '_rt.csv', 'w') as f:
        for i in range(cobiveco_mapped.shape[0]):
            f.write(str(cobiveco_mapped[i, 1]) + '\n')
    with open(figPath + meshName +  '_tm.csv', 'w') as f:
        for i in range(cobiveco_mapped.shape[0]):
            f.write(str(cobiveco_mapped[i, 2]) + '\n')
    with open(figPath + meshName +  '_tv.csv', 'w') as f:
        for i in range(cobiveco_mapped.shape[0]):
            f.write(str(cobiveco_mapped[i, 3]) + '\n')
...
def generatePurkinjeWithCobiveco(dataPath= 'metaData/DTI003/', meshName='DTI003', figPath='Figures_Clinical_PK/'):
    ...
    rv_dense_indexes[rv_apical_freewall_mask] = True # TODO CHANGE THIS TO 0.2 FOR THE RV!!!!!!!!!!!!! NO?
...
#     lv_dense_indexes[lv_apical_mask] = True # FUCK 2022
#     rv_dense_indexes[rv_apical_mask] = True # FUCK 2022
#     # Rule 6) Paraseptal regions of the heart are connected from apex to base through either [0.4/0.2, 0.1, 1., :] or  [0.4/0.2, 0.6, 1., :] LV/RV
#     lv_ant_paraseptalApex_index = int(np.argmin(np.linalg.norm(nodesCobiveco[lvnodes, :] - np.array([0.4, 0.6, 1., 0.]), ord=2, axis=1))) # [mid, paraseptal, endo, lv]
#     lv_post_paraseptalApex_index = int(np.argmin(np.linalg.norm(nodesCobiveco[lvnodes, :] - np.array([0.4, 0.1, 1., 0.]), ord=2, axis=1))) # [mid, paraseptal, endo, lv]
#     rv_ant_paraseptalApex_index = int(np.argmin(np.linalg.norm(nodesCobiveco[rvnodes, :] - np.array([0.2, 0.6, 1., 1.]), ord=2, axis=1))) # [mid, paraseptal, endo, rv]
...
def dtw_ecg(predicted, target_ecg, max_slope=1.5, w_max=10.):
    ...
    for conf_i in range(0, nParts):
        ...
        for lead_i in range(nLeads):
            ...
            cost_mat = np.zeros((n_timestamps_1, n_timestamps_2))
            ...
            for i in numba.prange(n_timestamps_1):
                for j in numba.prange(region[0, i], region[1, i]):
                    ...
                    # singals cannot be as similar anymore due to the difference in source models between real patients and EikonalDjikstraTet, -
            ...
            acc_cost_mat = np.zeros((n_timestamps_1, n_timestamps_2))
            ...
            dtw_list.append(math.sqrt(dtw_dist) )
...
# else:  # TODO DELETE THIS 2022/05/17
    return res[unique_index], [i + small_c * (n_timestamps_1-n_timestamps_2)**2/min(n_timestamps_1,n_timestamps_2) for i in dtw_list]
...
#         part_theta[:, scale_param] = np.exp(part_theta[:, scale_param])
        ...
        if self.experiment_output == 'simulated_ecg_normalised' or experiment_output == 'bsp':
    ...
    def sample(self, targetDiscrepancy):
        ...
        while looping:
            ...
            if (nUnique < unique_lim_nb) or (cuttoffDiscrepancy < targetDiscrepancy) or ((time.time()-inference_time)/3600 > max_time): # 2022/05/11 - Simplify strategy.
    ...
    def visualise(self, particle_indexes=None, prop_output_bad=None): # TODO: DELETE
        if particle_indexes is not None:
            ...
            # prop_data = self.simulator(population_unique, root_node_activation_indexes, rootNodeActivationTimes)  # TODO: UNCOMMENT 2022/05/17
            ...
            if self.experiment_output == 'simulated_ecg_normalised' or self.experiment_output == 'bsp':
                # prop_data = prop_data[unique_particles_reverse_index, :, :] # added on the 2022/05/11 # TODO: UNCOMMENT 2022/05/17
                # Compute the correlation coefficient of the simulated_ecg_normalised with best discrepancy - 2022/05/11
...
# To reproduce previous results with simultaneous activation at the root nodes, use root_node_times = np.ones(...)
def eikonal_ecg(population, rootNodesIndexes, rootNodesTimes):
    ...
    with pymp.Parallel(min(threadsNum, prediction_list.shape[0])) as p1:
        for conf_i in p1.range(prediction_list.shape[0]):
            ...
            # Root nodes will be activated at time =   root_node_times
            ...
            # root_node_times = np.around(root_node_times, decimals=4) # 02/12/2021 TODO: use only one decimal
            #
            # ## Initialise variables
            # activationTimes = np.zeros((nodesXYZ.shape[0],), np.float64)
            # visitedNodes = np.zeros((nodesXYZ.shape[0],), dtype=np.bool_)
            ...
            # tempTimes = np.zeros((nodesXYZ.shape[0],), np.float64) + 1000
            # params = population_unique[conf_i, :]
            # adjacentCost, eiknoal_rootNodes, eiknoal_rootActivationTimes = eikonal_one_ecg_part1(params, root_node_indexes, root_node_times)
            # time_sorting = np.argsort(eiknoal_rootActivationTimes) # 02/12/2021
            # print(eiknoal_rootNodes)
            # print(eiknoal_rootActivationTimes)
            # eiknoal_rootNodes = eiknoal_rootNodes[time_sorting] # 02/12/2021
            ...
            # with open('/users/jcamps/repos/EikonalDjikstraTet/Figures_Clinical_PK/DTI003_diff.ensi.ATMap', 'w') as f:
            #     f.write('EikonalDjikstraTet Ensight Gold --- Scalar per-node variables file\npart\n\t1\ncoordinates\n')
            #     for i in range(0, len(atmap)):
            #         f.write(str(atmap[i]) + '\n')
            #
            # with open('/users/jcamps/repos/EikonalDjikstraTet/Figures_Clinical_PK/DTI003_1.ensi.ATMap', 'w') as f:
            #     f.write('EikonalDjikstraTet Ensight Gold --- Scalar per-node variables file\npart\n\t1\ncoordinates\n')
            #     for i in range(0, len(activationTimes3)):
            #         f.write(str(activationTimes3[i]) + '\n')
            #
            # with open('/users/jcamps/repos/EikonalDjikstraTet/Figures_Clinical_PK/DTI003_2.ensi.ATMap', 'w') as f:
            #     f.write('EikonalDjikstraTet Ensight Gold --- Scalar per-node variables file\npart\n\t1\ncoordinates\n')
...
def pseudoBSP(ATMap):
    ...
    ECG[0, :] = (BSP[0, :] - BSP[1, :])
    ECG[1, :] = (BSP[2, :] - BSP[1, :])
    BSPecg = BSP - np.mean(BSP[0:3, :], axis=0) # changed from 2 to 3 2022/06/07
    BSP = None  # Clear Memory
    ECG[2:nb_leads, :] = BSPecg[4:nb_bsp, :]
...
def main_synthetic(args): # preserved as it was in 2021/05/22 for reproducibility of the the MIA paper
    ...
    target_type = args[1]
    if target_type == 'bsp':
        target_type = 'simulated_ecg_normalised'
    ...
    elif target_type == 'simulated_ecg_normalised':
        target_type = 'simulated_ecg_normalised'
    ...
    print(file_path_tag)
    load_target = False  # Needs to be false because these are EikonalDjikstraTet-generated
...
def main_clinical(args): # added on the 2021/05/21 to run inference on clinical data
    ...
    target_type = args[1]
    if target_type == 'simulated_ecg_normalised':
        rootNodeResolution = "newRV"
        target_snr_db = 0
    ...
    print(file_path_tag)
    load_target = True  # Needs to be false because these are EikonalDjikstraTet-generated
    ...
    consistency_iter = None
    # for consistency in range(consistency_count):
    for consistency in range(1, consistency_count):
        for meshName_val_aux in meshName_list:
            if run_count == run_iter:
                consistency_iter = consistency
                meshName_val = meshName_val_aux
